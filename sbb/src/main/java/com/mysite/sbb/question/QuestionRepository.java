...
public interface QuestionRepository extends JpaRepository<Question, Integer> {
	...
	// 날짜를 기준으로 검색 후 오름차순 정렬 (Asc): 1 --> 9, A --> Z, ㄱ --> ㅎ
	// 날짜를 기준으로 검색 후 내림차순 정렬 (Desc): 9 --> 1, Z --> A, ㅎ --> ㄱ
	...
	// 제목을 기준으로 오름차순 정렬
//	List<Question> findAllBySubjectAsc();
	
	// 제목을 기준으로 내림차순 정렬
//	List<Question> findAllBySubjectDesc();

	// Question (질문)테이블의 페이징 처리
